  ...
  try {
    ...
    if (!pedido.itemPedido?.length) {
      ...
      pedido.itemPedido.forEach((item) => {
        const produtoNome = item.produtoNome || "Produto desconhecido";
        ...
        carrinhoDiv.innerHTML += `
                    <div class="produto-carrinho">
                        ...
                        <p>Quantidade: <span id="quantidade-${
                          item.produtoId
                        }">${quantidade}</span></p>
                        <button class="btn btn-danger" onclick="alterarQuantidade(${
                          item.produtoId
                        }, -1, ${precoUnitario})">Remover -1</button>
                        <button class="btn btn-add" onclick="alterarQuantidade(${
                          item.produtoId
                        }, 1, ${precoUnitario})">Adicionar +1</button>
                    </div>
                `;
      });

      carrinhoDiv.innerHTML += `<div class="total">Total: R$${total.toFixed(
        2
      )}</div>`;
...
async function atualizarItemDoPedido(produtoId, novaQuantidade, preco) {
  ...
  const url = itemPedidoId
    ? `${API_BASE_URL}/api/ItensPedido/${itemPedidoId}`
    : `${API_BASE_URL}/api/ItensPedido`;
