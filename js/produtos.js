...
async function carregarPedidoAtual() {
  ...
  try {
    ...
    if (pedido.itemPedido && pedido.itemPedido) {
      ...
      quantidadeTotalCarrinho = Object.values(pedidoItens).reduce(
        (acc, item) => acc + (item.quantidade || 0),
        0
      );
      valorTotalCarrinho = Object.values(pedidoItens).reduce(
        (acc, item) => acc + (item.quantidade || 0) * (item.preco || 0),
        0
      );
  ...
  produtos.forEach((produto) => {
    ...
    obterImagemProduto(produto.nome).then((imgSrc) => {
      produtosContainer.appendChild(
        criarElementoProduto(produto, imgSrc, quantidadeInicial)
      );
    });
  });
}
...
function criarElementoProduto(produto, imgSrc, quantidadeInicial) {
  ...
  }g</p><p>Gorduras: ${produto.informacoesNutricionais.gorduras}g</p>
          </div>
      </div>
  `;
  return produtoDiv;
...
async function alterarQuantidade(produtoId, ajuste, preco) {
  ...
  if (novaQuantidade < 0) {
    exibirAlerta("A quantidade não pode ser negativa!", true); // true para alerta de erro
    return;
  }
...
async function adicionarItemAoPedido(produtoId, quantidade, preco) {
  ...
  try {
    ...
    pedidoItens[produtoId] = {
      itemPedidoId: data?.id || null,
      quantidade: quantidade,
      preco: preco,
    };
...
async function atualizarItemDoPedido(produtoId, novaQuantidade, preco) {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/ItensPedido/${itemPedidoId}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
...
async function carregarCarrinho() {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/Pedidos/${localStorage.getItem("pedidoId")}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
async function atualizarValorTotalPedido(pedidoId) {
  try {
    ...
    if (!response.ok)
      throw new Error("Erro ao obter dados do pedido para atualização.");
    ...
    const responseUpdate = await fetch(
      `${API_BASE_URL}/api/Pedidos/${pedidoId}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!responseUpdate.ok)
      throw new Error("Erro ao atualizar os dados do pedido.");
  } catch (error) {
...
function verificarImagemSimilar(nomeProduto) {
  const nomeFormatado = removerAcentos(
    nomeProduto.replace(/\s+/g, "").toLowerCase()
  );
  for (const arquivo of arquivosImagens) {
    const arquivoFormatado = removerAcentos(
      arquivo.replace(/\s+/g, "").toLowerCase()
    );
...
async function buscarImagemUnsplash(nomeProduto) {
  const produtoNomeTraduzido =
    traducoes[nomeProduto.toLowerCase()] || nomeProduto;
